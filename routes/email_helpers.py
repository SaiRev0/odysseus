...
def _init_scheduled_db():
    ...
    #   1. Only `replied_at` exists → rename it (SQLite >= 3.25)
    #   2. Only `sent_at` exists → nothing to do
    #   3. Both exist (intermediate state) → rebuild table dropping `replied_at`
    try:
        ar_cols = [
            r[1] for r in conn.execute("PRAGMA table_info(auto_reply_sent)").fetchall()
        ]
        has_replied = "replied_at" in ar_cols
        has_sent = "sent_at" in ar_cols
        if has_replied and not has_sent:
            conn.execute(
                "ALTER TABLE auto_reply_sent RENAME COLUMN replied_at TO sent_at"
            )
        elif has_replied and has_sent:
            # Both columns exist — rebuild without the legacy `replied_at` column
            conn.execute("""
                CREATE TABLE IF NOT EXISTS auto_reply_sent_new (
                    message_id TEXT,
                    owner      TEXT DEFAULT '',
                    rule_id    TEXT NOT NULL,
                    sent_at    TEXT NOT NULL,
            ...
            """)
            conn.execute("""
                INSERT OR IGNORE INTO auto_reply_sent_new (message_id, owner, rule_id, sent_at)
                SELECT message_id, owner, rule_id,
                       COALESCE(NULLIF(sent_at, ''), replied_at, '')
                FROM auto_reply_sent
            """)
            conn.execute("DROP TABLE auto_reply_sent")
            conn.execute("ALTER TABLE auto_reply_sent_new RENAME TO auto_reply_sent")
        elif not has_sent:
            conn.execute(
                "ALTER TABLE auto_reply_sent ADD COLUMN sent_at TEXT NOT NULL DEFAULT ''"
            )
    except Exception:
        pass
