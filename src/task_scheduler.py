    ...
    async def run_task_now(self, task_id: str, *, force: bool = False):
        ...
        async with self._executing_lock:
            if task_id in self._executing:
                return False
            self._executing.add(task_id)
        # bypass_model_slot=True so a manual "Run Now" starts immediately without
        # waiting for the interactive-idle gate — the user explicitly asked for it.
        asyncio.create_task(self._execute_task(task_id, bypass_model_slot=True))
        return True
