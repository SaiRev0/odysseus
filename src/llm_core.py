...
# As of mid-2025 OpenAI has migrated all models (including gpt-4o family)
# to max_completion_tokens; max_tokens is rejected with HTTP 400.
_MAX_COMPLETION_TOKENS_MODELS = {"o1", "o3", "o4", "gpt-4.5", "gpt-5", "gpt-4o"}
    ...
    while attempt < max_retries:
        ...
        try:
            ...
            if not r.is_success:
                ...
                # OpenAI: some model aliases (e.g. chat-latest) reject max_tokens
                # even when the static model-name list says they should not.
                # Detect this at runtime and retry once with max_completion_tokens.
                if (
                    r.status_code == 400
                    ...
                    logger.info(
                        f"Retrying {target_url} with max_completion_tokens "
